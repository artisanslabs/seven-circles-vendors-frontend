<template>
  <div class="list">
    ...
    <vehicle-modal :dialog-visible="showModal" :title="modalTitle" :vehicle="modalData" @closeModal="isModalClosed" />
    <delete-alert
...
export default {
  name: 'VehiclesPage',
  components: { VehicleModal, DeleteAlert },
  data () {
    return {
      perPage: false,
      multiItems: false,
      ...
      headers: [
        ...
        {
          text: this.$t('vehicles.vehicle_number'),
          sortable: false,
          value: 'vin_number'
        },
  ...
  async fetch ({ store, params }) {
    await store.dispatch('global/fetchVehiclesList', {
      type: 'vehicles'
    })
  },
  head: {
  ...
  computed: {
    response () {
      return { ...this.$store.state.global.vehicles }
    },
    tableData () {
  ...
  methods: {
    ...
    async handleDelete (payload) {
      if (payload.value) {
        await GlobalServices.delete(this.$axios, { id: this.$route.params.id, item_id: this.deletedItem.id, type: 'vehicles' }).then((resData) => {
          if (resData.success) {
            this.fetch()
          }
    ...
    fetch (pageNum) {
      ...
      this.filters.page = this.perPage
        ? (this.filters.page = 1)
        : this.filters.page
      this.$store.dispatch('global/fetchVehiclesList', { type: 'vehicles', ...this.filters }).then(() => {
        this.loading = false
        this.perPage = false
      })
    },
