      ...
      <div>
        <v-form ref="form" lazy-validation>
          <v-row>
            ...
            <v-col cols="12" sm="6">
              <div class="text-start mb-2">
                <span>{{ $t('vehicles.vehicle_number') }}</span>
                <span class="red-color">{{ $t('v.star') }}</span>
              </div>
              <v-text-field
                v-model="form.vin_number"
                placeholder="اكتب رقم المركبة"
                type="number"
                hide-spin-buttons
                :rules="[requiredRules]"
                validate-on-blur
...
export default {
  ...
  computed: {
    pageTitle () {
      return this.title === 'add vehicle'
        ? this.$t('vehicles.create')
        : this.$t('vehicles.update')
    },
    cities () {
  ...
  watch: {
    ...
    vehicle () {
      if (this.title !== 'add vehicle') {
        this.form = { ...this.vehicle }
        this.oldForm = { ...this.vehicle }
      }
    }
  },
  methods: {
    async submit () {
      ...
      if (valid) {
        ...
        const payload = { formData, type: 'vehicles' }
        let action = 'create'
