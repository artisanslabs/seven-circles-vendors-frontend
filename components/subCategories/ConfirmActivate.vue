    ...
    <v-dialog v-model="showAlert" class="activation-alert" max-width="470" persistent>
      <v-card class="pa-5 activation-alert">
        <div v-if="item.status" class="text-center pt-6 mb-6">
          <p class="mb-n4">
            تأكيد إعادة تفعيل الصنف الفرعي ؟
          </p><br>
          <!-- <small class="note">
        ...
        <div v-else class="text-center pt-6 mb-6">
          <p class="mb-n4">
            تأكيد إلغاء تفعيل الصنف الفرعي ؟
          </p><br>
          <!-- <small class="note">
...
export default {
  ...
  methods: {
    async confirm () {
      ...
      const payload = { id: this.item.id, statusType: this.item.status ? 'activate' : 'deactivate', formData, type: 'sub-categories' }
