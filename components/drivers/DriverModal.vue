...
export default {
  ...
  computed: {
    pageTitle () {
      return this.title === 'add driver'
        ? this.$t('drivers.create')
        : this.$t('drivers.update')
    },
    cities () {
  ...
  watch: {
    ...
    driver () {
      if (this.title !== 'add driver') {
        this.form = { ...this.driver }
        this.oldForm = { ...this.driver }
      }
    }
  },
  methods: {
    async submit () {
      ...
      if (valid) {
        ...
        for (const key in this.form) {
          if (this.form[key] !== this.oldForm[key] && key !== 'vehicle') {
            formData.append(key, this.form[key])
          }
        }
        formData.append('vehicle_id', this.form.vehicle.id)

        const payload = { type: 'drivers', formData }
        let action = 'create'
        // for Updating driver
        if (this.title !== 'add driver') {
          formData.append('_method', 'patch')
          payload.id = this.driver.id
          action = 'update'
        }
